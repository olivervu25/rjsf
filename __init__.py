...
def jsf(x0: SystemState, rates, stoich, t_max, **kwargs) -> Trajectory:
    ...
    elif kwargs['method'] == 'tau-operator-splitting':
        result = JumpTauSwitchFlowSimulator(x0, rates, stoich, t_max, kwargs['config'])
    else:
...
def JumpTauSwitchFlowSimulator(
        x0: SystemState,
        rates: Callable[[SystemState, Time], List[float]],
        stoich: Dict[str, Any],
        t_max: Time,
        options: Dict[str, Any]) -> Trajectory:
    """
    Prototype Tau-ODE hybrid simulator.

    First version:
    - compartments below TauThreshold are treated as Tau-leaping;
    - compartments above TauThreshold are treated as ODE / flow;
    - reactions touching any Tau compartment are Tau reactions;
    - all other reactions are Flow reactions.

    This version does not include exact CTMC jumps yet.
    """
    ...
    if "TauThreshold" in options and options["TauThreshold"] is not None:
        TauThreshold = options["TauThreshold"]
    else:
        TauThreshold = options["SwitchingThreshold"]

    TauDebug = bool(options.get("TauDebug", False))
    TauDebugEvery = int(options.get("TauDebugEvery", 20))
    TauDebugMax = int(options.get("TauDebugMax", 30))
    TauDebugCount = 0

    # Identify which compartments are involved in each reaction.
    if 'SwitchingType' not in options or options['SwitchingType'] is None or options['SwitchingType'] == 'default':
        NuComp = [[value != 0 for value in row] for row in nu]
        ReactComp = [[value != 0 for value in row] for row in nuReactant]
        compartInNu = [[value != 0 for value in row] for row in MatrixPlusAB(NuComp, ReactComp)]
        ...
        raise RuntimeError("Unknown SwitchingType")

    # Store trajectory.
    X = [[CompartmentValue(x0[i])] for i in range(nCompartments)]
    TauArr = [Time(0.0)]
    EventType = [0]
    ...
    while t < t_max:
        ...
        Dtau = Time(min(dt, t_max - t))

        Xprev = SystemState([x[iters] for x in X])
        Props = rates(Xprev, t)

        # ------------------------------------------------------------
        # 1. Classify compartments: Tau or Flow
        # ------------------------------------------------------------
        compartmentMode = []

        for i in range(nCompartments):
            if Xprev[i] <= TauThreshold[i] and Xprev[i] == round(Xprev[i]):
                compartmentMode.append("tau")
            else:
                compartmentMode.append("flow")

        # ------------------------------------------------------------
        # 2. Classify reactions: Tau if touching any Tau compartment
        # ------------------------------------------------------------
        reactionMode = ["flow"] * nRates

        for reactionIdx in range(nRates):
            for compIdx in range(nCompartments):
                if compartInNu[reactionIdx][compIdx] and compartmentMode[compIdx] == "tau":
                    reactionMode[reactionIdx] = "tau"

        if TauDebug and iters % TauDebugEvery == 0 and TauDebugCount < TauDebugMax:
            print(
                "t={:.3f}, X={}, compartmentMode={}, reactionMode={}".format(
                    float(t),
                    [round(float(x), 3) for x in Xprev],
                    compartmentMode,
                    reactionMode
                )
            )
        ...
        # ------------------------------------------------------------
        # 3-5. ODE / Flow update + Tau-leaping update with step halving
        # ------------------------------------------------------------
        maxTauRetries = int(options.get("MaxTauRetries", 20))
        minDtau = float(options.get("MinTauStep", 1e-10))
        ...
        while not acceptedStep:
            ...
            for compIdx in range(nCompartments):
                ...
                for reactionIdx in range(nRates):
                    if reactionMode[reactionIdx] == "flow":
                        derivative += Props[reactionIdx] * nu[reactionIdx][compIdx]

                dXdt.append(derivative)

            Xcurr = [
                Xprev[i] + DtauTry * dXdt[i]
                for i in range(nCompartments)
            ]

            # --------------------------------------------------------
            # 4. Tau-leaping update
            # --------------------------------------------------------
            for reactionIdx in range(nRates):

                if reactionMode[reactionIdx] == "tau":

                    lam = Props[reactionIdx] * DtauTry
                    lam = max(float(lam), 0.0)

                    nFirings = np.random.poisson(lam)

                    for compIdx in range(nCompartments):
                        Xcurr[compIdx] += nFirings * nu[reactionIdx][compIdx]

            # --------------------------------------------------------
            # 5. Negative population check
            # --------------------------------------------------------
            if not any(value < 0 for value in Xcurr):
                acceptedStep = True
            else:
                retryCount += 1
                DtauTry = Time(DtauTry / 2.0)

                if TauDebug:
                    print(
                        "Tau step rejected at t={:.3f}; retry={}, new Dtau={:.6g}".format(
                            float(t),
                            retryCount,
                            float(DtauTry)
                        )
                ...
                if retryCount > maxTauRetries or float(DtauTry) < minDtau:
                    raise RuntimeError(
                        "Tau-leap produced a negative population even after "
                        "step-halving retries."
                    )

        # Use the accepted step size from now on.
        Dtau = DtauTry

        # ------------------------------------------------------------
        # 6. Round Tau compartments
        # ------------------------------------------------------------
        for compIdx in range(nCompartments):
            if compartmentMode[compIdx] == "tau":
                Xcurr[compIdx] = round(Xcurr[compIdx])
        ...
        for compIdx in range(nCompartments):
            X[compIdx].append(CompartmentValue(Xcurr[compIdx]))

        TauArr.append(t)
        EventType.append(-2)  # -2 means Tau-ODE step for now
